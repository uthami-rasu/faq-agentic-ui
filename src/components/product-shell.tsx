...
export function ProductShell({ initialData, initialView }: { initialData?: InitialAppData; initialView?: ViewKey }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <div className="content-wrap">
        <AnimatePresence mode="wait"><motion.div key={`${view}-${view === "organization" ? organizationTab : view === "agent" ? agentTab : ""}`} initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: .2 }}>
          ...
          {view === "orchestrator" && <OrchestratorPage organizationId={selectedOrganization.id} agents={organizationAgents} organizationName={selectedOrganization.name} notify={notify} onSaved={(active) => setDashboardData((current) => ({ ...current, [selectedOrganization.id]: { ...currentDashboard, orchestrator: { active } } }))}/>}
    ...
    <AnimatePresence>{showCreate && <CreateAgentModal close={() => setShowCreate(false)} add={createAgent} organizationId={selectedOrganization.id} initialDocuments={selectedOrganization.id === ssrOrganizationId ? initialData?.documentsPage : undefined}/>}</AnimatePresence>
