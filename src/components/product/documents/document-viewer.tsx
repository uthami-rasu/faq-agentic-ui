...
import { Activity, Download, ExternalLink, FileCode2, FileText, LoaderCircle, ShieldCheck, X } from "lucide-react";
...
export function DocumentViewer({ organizationId, document: selectedDocument, close }: Props) {
  ...
  useEffect(() => { setPdfReady(false); }, [baseUrl]);

  return createPortal(<div className="document-viewer-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className="document-viewer" role="dialog" aria-modal="true" aria-label={`Preview ${selectedDocument.file_name}`}><header className="document-viewer-header"><div className="document-viewer-title"><span className="document-viewer-icon">{kind === "markdown" ? <FileCode2 size={20}/> : <FileText size={20}/>}</span><div><h2>{selectedDocument.file_name}</h2><p><span>{kind.toUpperCase()}</span>{formatBytes(selectedDocument.size_bytes)} · Private organization document</p></div></div><nav className="document-viewer-actions" aria-label="Preview actions"><button className={view === "history" ? "active" : ""} onClick={() => setView((current) => current === "preview" ? "history" : "preview")}><Activity size={15}/><span>{view === "history" ? "Preview" : "History"}</span></button><a href={baseUrl} download={selectedDocument.file_name}><Download size={15}/><span>Download</span></a><button autoFocus className="document-viewer-close" aria-label="Close preview" title="Close preview" onClick={close}><X size={19}/></button></nav></header><div className="document-viewer-trust"><span><ShieldCheck size={14}/><b>Secure preview</b><small>Streamed through your authenticated organization</small></span><span><kbd>ESC</kbd> to close</span></div>{view === "history" ? <main className="document-viewer-history"><ActivityTimeline organizationId={organizationId} entityType="DOCUMENT" entityId={selectedDocument.id} title={`${selectedDocument.file_name} history`}/></main> : <main className={`document-viewer-content viewer-${kind}`}>{kind === "pdf" ? <><iframe src={baseUrl} title={selectedDocument.file_name} onLoad={() => setPdfReady(true)}/>{!pdfReady && <PreviewLoading label="Loading PDF preview" detail="Preparing pages and the browser document viewer…"/>}</> : kind === "docx" ? <RichDocxPreview sourceUrl={baseUrl}/> : textQuery.isLoading ? <PreviewLoading label="Loading preview" detail="Fetching the protected document content…"/> : textQuery.isError ? <div className="viewer-state error"><ExternalLink size={26}/><b>Preview unavailable</b><p>{textQuery.error instanceof Error ? textQuery.error.message : "The document could not be opened."}</p><a href={baseUrl} download={selectedDocument.file_name}>Download original file</a></div> : kind === "markdown" ? <MarkdownPreview source={textQuery.data ?? ""}/> : <pre className="plain-text-preview">{textQuery.data}</pre>}</main>}</section></div>, document.body);
}

function PreviewLoading({ label, detail }: { label: string; detail: string }) {
  return <div className="viewer-loading-overlay" role="status" aria-live="polite"><span><LoaderCircle className="spin" size={24}/></span><b>{label}</b><p>{detail}</p><i><em/></i></div>;
}
