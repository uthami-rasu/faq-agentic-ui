...
import { ArrowUpRight, Bot, Check, ChevronDown, ChevronRight, Files, LayoutDashboard, Menu, Moon, MoreHorizontal, Network, Pencil, Plus, Search, Settings, Sparkles, Sun, X } from "lucide-react";
...
export function AppSidebar(props: SidebarProps) {
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <div className="sidebar-bottom"><aside className="sidebar-product-card"><i className="product-orb product-orb-one"/><i className="product-orb product-orb-two"/><span className="sidebar-product-kicker"><Sparkles size={12}/>Arffy intelligence</span><strong>One assistant.<br/>Every FAQ agent.</strong><p>Let the AI Orchestrator route each question to the right knowledge.</p><button onClick={() => changeView("orchestrator")}>Explore Orchestrator <ArrowUpRight size={14}/></button></aside><button className="profile-row"><span className="profile-avatar">OS</span><span><strong>Olivia Stone</strong><small>olivia@acme.com</small></span><MoreHorizontal size={17}/></button></div>
    </aside>
  </>;
}
