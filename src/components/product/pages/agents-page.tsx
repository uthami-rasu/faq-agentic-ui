...
export function AgentsPage({ agents, organizationId, search, setSearch, ssrPage, ssrQuery, create, openAgent, duplicateAgent, deleteAgent, notify, canCreate, canEdit, canDelete }: AgentsPageProps) {
  const pageSize = 6;
  ...
  return <div className="agents-page">
    <PageHeading eyebrow="Knowledge team" title="FAQ Agents" description="Build specialized AI agents grounded in your company knowledge."><button className="secondary-button"><SlidersHorizontal size={16}/> Filters</button>{canCreate && <button className="primary-button" onClick={create}><Plus size={17}/> Create agent</button>}</PageHeading>
    ...
    <div className={`agents-grid ${layout === "list" ? "list-view" : ""}`} aria-busy={isPending && !useMockData}>
      ...
      {!showShimmer && canCreate && visibleAgents.length > 0 && page === 1 && !search && <button className="new-agent-card" onClick={create}><span><Plus size={22}/></span><strong>Create a new agent</strong><small>Train an expert on a new topic</small></button>}
    </div>
    {visibleAgents.length === 0 && !isPending && <div className="agents-empty"><Search size={22}/><h2>No agents found</h2><p>{search ? `No FAQ agents match “${search}”.` : canCreate ? "Create your first FAQ agent to get started." : "No FAQ agents are available to this organization."}</p>{search ? <button className="secondary-button" onClick={() => setSearch("")}>Clear search</button> : canCreate && <button className="primary-button" onClick={create}><Plus size={16}/> Create agent</button>}</div>}
