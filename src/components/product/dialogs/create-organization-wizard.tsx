...
export function CreateOrganizationWizard({ close, complete }: Props) {
  ...
  const finish = async () => {
    ...
    setSaving(true);
    await complete({ name: name || "New organization", description: description || "A new QueryDesk organization.", category: "Organization", initials: (name || "N").charAt(0).toUpperCase(), color: "violet" }, { name: agentName, description: agentDescription, documentAdded });
    setSaving(false);
  };
